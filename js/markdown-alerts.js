export function parseMarkdownAlerts(content) {
    // Match [!NOTE] followed by newline and content
    const alertRegex = />\s*\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\]\s*\n>\s*(.*?)(?=\n(?!>)|$)/gs;
    
    return content.replace(alertRegex, (match, type, text) => {
        const typeLC = type.toLowerCase();
        const icon = getAlertIcon(typeLC);
        
        return `<div class="markdown-alert markdown-alert-${typeLC}" dir="auto">
            <p class="markdown-alert-title" dir="auto">${icon}${type}</p>
            <p dir="auto">${text.trim()}</p>
        </div>`;
    });
}
